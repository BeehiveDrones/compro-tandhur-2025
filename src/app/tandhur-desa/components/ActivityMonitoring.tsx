...
export default function ActivityMonitoring() {
  const images = ["/desa/frame-01.png", "/desa/frame-02.png", "/desa/frame-03.png"];
      ...
      <div className="flex justify-center py-8 md:py-12">
        ...
        >
          ...
          <div className="flex gap-2 mt-4 z-10">
            {images.map((_, index) => (
              <button key={index} onClick={() => setCurrent(index)} className={`w-3 h-3 md:w-4 md:h-4 rounded-full transition-all duration-300 ${current === index ? "bg-[#F3D34A] scale-125" : "bg-gray-300 hover:bg-gray-400"}`} />
            ))}
          </div>
