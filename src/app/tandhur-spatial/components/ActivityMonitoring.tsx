...
export default function ActivityMonitoring() {
  const images = ["/spatial/frame-01.png", "/spatial/frame-02.png", "/spatial/frame-03.png"];
      ...
      <div className="flex justify-center py-8 md:py-12">
        ...
        >
          ...
          <div className="flex gap-2 mt-4 z-10">
            {images.map((_, index) => (
              <button key={index} onClick={() => setCurrent(index)} className={`w-3 h-3 md:w-4 md:h-4 rounded-full transition-all duration-300 ${current === index ? "bg-[#FF5757] scale-125" : "bg-gray-300 hover:bg-gray-400"}`} />
            ))}
          </div>
