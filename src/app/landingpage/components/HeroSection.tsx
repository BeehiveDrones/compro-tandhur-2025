...
const HeroSection = () => {
  ...
  return (
    <section className="py-12 px-4 bg-white text-center overflow-hidden -mt-10 ">
      {/* Judul */}
      <motion.h2 initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} className="text-3xl font-bold text-black font-josefin-sans mb-4">
        TANDHUR APPS SERIES
      </motion.h2>

      {/* Deskripsi */}
      <motion.p initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} viewport={{ once: true }} className="text-gray-700 font-nunito-sans max-w-3xl mx-auto mb-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8 max-w-6xl mx-auto mt-20 ">
        ...
        {[
          ...
          >
            ...
            <p className="text-gray-700  font-nunito-sans">{item.text}</p>
          </motion.div>
        ))}
      </div>
    </section>
